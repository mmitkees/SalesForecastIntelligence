...
echo -e "${YELLOW}Note: This will run deploy.sh interactively on the server.${NC}"
echo ""

# Run deploy.sh interactively so user can make choices
ssh -t $SSH_OPTS "$REMOTE_SERVER_USER@$REMOTE_SERVER_IP" "cd $REMOTE_APP_DIR && bash deploy.sh"
